...
class ProductValidateDateList extends TPage
{
    private $form; // form
    ...
    private static $formName = 'form_ProductValidateDateList';
    ...
    public function __construct($param = null)
    {
        ...
        $btn_onshow = $this->form->addAction("Cadastrar", new TAction(['ProductValidateDateForm', 'onShow']), 'fas:plus #69aa46');
        ...
        $action_onDelete = new TDataGridAction(array('ProductValidateDateList', 'onDelete'));
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['ProductValidateDateList', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-csv #00b894' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['ProductValidateDateList', 'onExportXls'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['ProductValidateDateList', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['ProductValidateDateList', 'onExportXml'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-code #95a5a6' );
    ...
    { 
        if(isset($param['delete']) && $param['delete'] == 1)
        {
            try
            {
                ...
                // instantiates object
                $object = new ProductValidateDate($key, FALSE); 
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->lote) AND ( (is_scalar($data->lote) AND $data->lote !== '') OR (is_array($data->lote) AND (!empty($data->lote)) )) )
        {
            ...
            $filters[] = new TFilter('lote', 'like', "%{$data->lote}%");// create the filter 
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for ProductValidateDate
