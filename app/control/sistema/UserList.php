...
class UserList extends TPage
{
    ...
    private static $formName = 'form_UserList';
    ...
    public function __construct($param = null)
    {
        ...
        $btn_onshow = $this->form->addAction("Cadastrar", new TAction(['UserForm', 'onShow']), 'fas:plus #69aa46');
        ...
        $action_onDelete = new TDataGridAction(array('UserList', 'onDelete'));
        ...
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['UserList', 'onExportXls'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['UserList', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['UserList', 'onExportXml'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-code #95a5a6' );
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->system_user) AND ( (is_scalar($data->system_user) AND $data->system_user !== '') OR (is_array($data->system_user) AND (!empty($data->system_user)) )) )
        {
            ...
            $filters[] = new TFilter('system_user', '=', $data->system_user);// create the filter 
        }
