...
class PaymentMethodList extends TPage
{
    private $form; // form
    ...
    private static $activeRecord = 'PaymentMethod';
    private static $primaryKey = 'id';
    private static $formName = 'form_PaymentMethodList';
    ...
    public function __construct($param = null)
    {
        ...
        $btn_onshow = $this->form->addAction("Cadastrar", new TAction(['PaymentMethodForm', 'onShow']), 'fas:plus #69aa46');
        ...
        $action_onDelete = new TDataGridAction(array('PaymentMethodList', 'onDelete'));
        ...
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['PaymentMethodList', 'onExportXls'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['PaymentMethodList', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['PaymentMethodList', 'onExportXml'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-code #95a5a6' );
    ...
    { 
        if(isset($param['delete']) && $param['delete'] == 1)
        {
            try
            {
                ...
                // instantiates object
                $object = new PaymentMethod($key, FALSE); 
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->method) AND ( (is_scalar($data->method) AND $data->method !== '') OR (is_array($data->method) AND (!empty($data->method)) )) )
        {
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for PaymentMethod
